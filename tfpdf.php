...
class tFPDF
{
	...
	protected $LI = '';
	protected $BR = 0;
	protected $COLONNE = false;
	protected $coly1= 0;
	protected $coly2= 0;
	protected $oldMargeL= 0;
	protected $oldMargeR= 0;
	protected $goutiere = 10;
	...
	*******************************************************************************/
	...
	function Row($data)
	{
		...
		for($i=0;$i<count($data);$i++)
		{
			...
			$this->Rect($x,$y,$w,$h);
			//Imprime le texte
			$tabTempAlign = explode("##", $data[$i]);

			$this->MultiCell($w,4,trim($tabTempAlign[1]),0,trim($tabTempAlign[0]));
	...
	function WriteHTML($html)
	{
		...
		foreach($a as $i=>$e)
		{
			if($i%2==0)
			{
				...
				if ($this->HREF) {
					$this->PutLink($this->HREF,$this->traitText(utf8_decode($e)));
				} else if ($this->BULLET || $this->SUBBULLET) {
					$oldMarge = $this->lMargin;
					$oldFontSize = $this->FontSizePt;
					$x = $this->lMargin; $puce = chr(127); $this->SetFontSize(14);
					if ($this->SUBBULLET){
						$this->SetFontSize(6);
						$puce = "o";
						$x += 6;
					...
					$this->Write(5,$this->traitText(utf8_decode($e)));
					$this->SetLeftMargin($oldMarge);
					$this->BULLET = "";
					$this->SUBBULLET = "";
				} else {
					$this->Write(5,$this->traitText(utf8_decode($e)));
	...
	{
		// Balise ouvrante
		if ($tag=='ORANGE' || $tag=='BLUE' || $tag=='BLACK' || substr($tag, 0, 2) == "FS" || $tag=='FZ10' || $tag=='FZ12' || $tag=='FZ7' || $tag=='FZ8') {
		...
		elseif($tag=='BULLET')
			$this->BULLET = "yes!";
		elseif($tag=='SUBBULLET')
			$this->SUBBULLET = "yes!";
		elseif(substr($tag, 0, 2)=='BR'){
			$i = -1;
			if(strlen($tag) > 2){
				$i = intval(substr($tag, 2), 10);
			}
			$this->BR = $i;
		}elseif($tag=='COLONNE'){

			$this->COLONNE = true;
			$this->coly1 = $this->getY();
			$this->oldMargeL = $this->lMargin;
			$this->oldMargeR = $this->rMargin;
			$mergeR = (($this->w - $this->oldMargeL - $this->oldMargeR + $this->goutiere) / 2) + $this->oldMargeR;
			$this->SetRightMargin($mergeR);

		}elseif($this->COLONNE && $tag=='SECTION'){

			$this->SetRightMargin($this->oldMargeR);
			$mergeL = (($this->w - $this->oldMargeL - $this->oldMargeR + $this->goutiere) / 2) + $this->oldMargeL;
			$this->SetLeftMargin($mergeL);
			$this->setY($this->coly1);
	...
	{
		if ($tag=='B' || $tag=='I' || $tag=='U' || $tag=='ORANGE' || substr($tag, 0, 2) == "FS" || $tag=='BLUE' || $tag=='BLACK' || $tag="FZ10" || $tag="FZ12" || $tag="FZ7" || $tag="FZ8") {
		...
		if (($tag == "COLONNE") && (is_string($tag))) {

			$this->COLONNE = false;
			$this->SetLeftMargin($this->oldMargeL);
			if($this->getY() < $this->coly2) $this->setY($this->coly2);
	...
	function SetStyle($tag, $enable)
	{
		...
		if ($tag == "ORANGE") {
			$this->SetTextColor(244, 131, 11);
		} else if ($tag == "BLUE") {
			$this->SetTextColor(9, 3, 119);
		} else if ($tag == "BLACK") {
			$this->SetTextColor(0, 0, 0);
		} else if (substr($tag, 0, 2) == "FS") {
			$size = intval(substr($tag, 2), 10);
	...
	function Header()
	{
		...
		$customHd = $this->customHd;
		$this->AddFont('roboto-bold','','roboto-bold.ttf',true);
		$this->SetFont('roboto-bold', '', 13, '', true);

		if (($customHd == "contrat1") || ($customHd == "contrat2") || ($customHd == "contrat3")) {

			$this->SetXY(0,0);
			$this->Image('../media/logo-sc.jpg',10,8,50);

			$this->SetXY(25, 10);
			if ($customHd == "contrat1") {
				$headerText = "Contrat d'engagement de service civique";
			} else if ($customHd == "contrat2") {
				$headerText = "Convention de mise à disposition d'un volontaire";
			} else if ($customHd == "contrat3") { // Mona lisa
				$headerText = "Convention de mise à disposition d'un volontaire";
			}
			$this->Cell(0,10,$this->traitText($headerText),0,0,'C');
	...
	{
		$this->SetY(-22);
		$customFt = $this->customFt;
		if ($customFt == "facture") {
			$this->AddFont('roboto-regular','','roboto-regular.ttf',true);
			$this->SetFont('roboto-regular', '', 6, '', true);

			$this->Cell(0,10,$this->traitText('Union française des centres de vacances et de loisirs ∙ Association nationale reconnue d’utilité publique'),0,0,'C');
			$this->ln(3);
			$this->Cell(0,10,$this->traitText('Siège : Immeuble l\'Artois - 11 rue de Cambrai - CS90042 – 75019  Paris – SIREN 775 685 621'),0,0,'C');
			$this->ln(3);
			$this->Cell(0,10,$this->traitText('Agréments tourisme AG n° 075 96 00 29, vacances adaptées organisées n°2006-853, éducation populaire'),0,0,'C');
			$this->ln(3);
			$this->Cell(0,10,$this->traitText('Habilitation nationale formation Bafa/d ∙ Certification OPQF'),0,0,'C');

			$this->SetXY(-18, -18);
			$this->Cell(0,10,'Page '.$this->PageNo().' / {nb} '.$this->AliasNbPages(),0,0,'C');

		} else if ($customFt == "contrat") {
			$this->AddFont('roboto-bold-italic','','roboto-bold-italic.ttf',true);
			$this->SetFont('roboto-bold-italic', '', 8, '', true);
			$this->SetTextColor(0, 0, 0);
			$this->SetY(-15);
			$this->Cell(0,10,'Service Civique – page '.$this->PageNo().' / {nb} '.$this->AliasNbPages(),0,0,'C');
		} else if ($customFt == "entretien") {
			$this->AddFont('roboto-italic','','roboto-italic.ttf',true);
			$this->SetFont('roboto-italic', '', 7, '', true);
			$this->SetTextColor(0, 0, 0);
			$this->SetY(-15);
			$this->Cell(0,10,'Service Civique – page '.$this->PageNo().' / {nb} '.$this->AliasNbPages(),0,0,'C');
		}
	}
	function CustomFooter($customValue) {
		$this->customFt = $customValue;
	}
	function CustomHeader($customValue2) {
		$this->customHd = $customValue2;
	}
	function DrawGrid()
	{
		if($this->grid===true){
	...
	*******************************************************************************/
	...
	public static function traitText($textU, $unicode=false) {
		// myLog($unicode);
		$nbsp = mb_convert_encoding(chr(160), 'UTF-8', 'HTML-ENTITIES');
		$searchReplace = [
			"&bull;" => "•",
			"∙" => "•",
			"’" => "'",
		...
		$textU = htmlspecialchars_decode(str_replace("…", "...", $textU), ENT_QUOTES|ENT_HTML401);
		$textU =  str_replace(array_keys($searchReplace), array_values($searchReplace), $textU);
		if($unicode) $textU = utf8_decode($textU);
		return $textU;
	}
